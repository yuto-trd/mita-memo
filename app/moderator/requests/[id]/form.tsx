...
import { FormControl, FormErrorMessage, FormLabel, Heading, Input, NumberDecrementStepper, NumberIncrementStepper, NumberInput, NumberInputField, NumberInputStepper, Select, Text, Textarea } from "@chakra-ui/react";
import { SubmitButton } from "@/app/add-record/submit-button";
import { Tables } from "@/types/supabase";
import { ChangeEvent, useRef } from "react";
import imageCompression from "browser-image-compression";

export function Form({ item }: { item: Tables<'requests'> & { response4request: Tables<"response4request">[] } }) {
        ...
        <div className="mt-3 flex flex-col items-center">
            <div className="flex-1 flex flex-col w-full px-4 sm:max-w-md justify-center gap-2">
                <form
                    ...
                    <Heading size="lg" className="mb-2">リクエストを処理</Heading>
                    <Text>{item.name}</Text>
                    ...
                    <FormControl>
                        ...
                        <Select
                            name="status"
                            defaultValue={item.response4request?.[0]?.reject === true
                                ? "reject" : item.response4request?.[0]?.reject === false
                                    ? "approve" : "none"}>
